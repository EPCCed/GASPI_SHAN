    ...
    )
{
	...
	for (int by = 1; by < nby - 1; ++by) 
	{
		...
		if (rank == 0)
		{
		    ...
		    dep = (stage[type_id].sendLast == stage[type_id].solve - 1);
		    TrySendLastComputeRow(conf, rank, type_id
					  , dep
					  , &stage[type_id].sendLast
			);

		    dep = (stage[type_id].testLast == stage[type_id].sendLast - 1);
		...
		else if (rank == rank_size - 1)
		{
		    ...
		    dep = (stage[type_id].sendFirst == stage[type_id].counter - 1);
		    TrySendFirstComputeRow(conf, rank, type_id
					   , dep
					   , &stage[type_id].sendFirst
			);

		    dep = (stage[type_id].testFirst == stage[type_id].sendFirst - 1);
