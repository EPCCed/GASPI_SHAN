...
{  
  ...
  MPI_Comm_split_type (MPI_COMM_WORLD
	...
	nelem_recv[i] = cd->recvcount[k];
	send_sz[i]    =  max_elem_sz * szd;
	recv_sz[i]    =  max_elem_sz * szd;

	for(j = 0; j < cd->sendcount[k]; j++)
	{
	    int pnt = cd->sendindex[k][j];
	    ptrdiff_t diff =  &(grad[pnt][0][0]) - &(grad[0][0][0]); 
	...
	for(j = 0; j < cd->recvcount[k]; j++)
	{
	    int pnt = cd->recvindex[k][j];
            ptrdiff_t diff =  &(grad[pnt][0][0]) - &(grad[0][0][0]);
	    recv_offset[i * max_nelem_recv + j] = diff;
	}
    }
